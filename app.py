...
def calculate_distance(start_mileage, end_mileage):
    """
    Distance is ALWAYS calculated as:
    ...
    try:
        start = float(start_mileage)
        end = float(end_mileage)
...
def authenticate_user(
    ...
    for _, row in users.iterrows():
        if (
            ...
            == username
            and str(row.get("password", "")).strip()
            == password
...
def available_vehicles_for_interval(
    ...
    """Find vehicles available for the exact requested time."""
...
def transfer_reminder_dialog():
    ...
    st.warning(
        ...
        "but it has NOT been transferred to "
        "the company Google Sheet yet."
    )
    ...
    st.info(
        "The request is currently stored only "
        "in this browser session. Managers and HR "
        "will not see it until you transfer it."
    )
...
def _transfer_pending_request():
    ...
    if st.session_state.get(
        ...
        st.warning(
            "The request is already being transferred. "
            "Please wait."
        )
...
def employee_portal():
    ...
    st.caption(
        "Enter the date and exact time you need "
        "the vehicle. The Vehicle Allocator will "
        "select an available vehicle for your "
        "requested time."
    )
    ...
    with st.form(
        ...
        with col2:
            ...
            if (
                ...
                st.error(
                    "The requested duration extends "
                    "into the next day. Please choose "
                    "an earlier start time or shorter duration."
                )

        st.divider()

        st.info(
            "Your requested date and time will be "
            "sent for approval. After approval, the "
            "Vehicle Allocator will select a vehicle "
            "and its permanently assigned driver that "
            "are available during this exact period."
        )
    ...
    if prepared:
        ...
        if (
            ...
            st.error(
                "The requested vehicle time cannot "
    ...
    st.warning(
        "Your request is ready, but it has not "
        "been sent to Google Sheets. Click the "
        "button below to officially submit it."
    )
    ...
    for _, row in pending.iterrows():
        ...
        with st.expander(
            ...
            with col2:
                ...
                st.info(
                    "The Vehicle Allocator will select "
                    "an available vehicle for this "
                    "requested time."
                )
    ...
    for _, row in pending.iterrows():
        ...
        with st.expander(
            ...
            if (
                ...
                st.error(
                    "The travel date, requested start "
                    "time, or duration is invalid."
                )
            ...
            selected_label = st.selectbox(
                ...
                vehicle_labels,
                key=(
                    f"allocator_vehicle_{request_id}"
                ),
            )
            ...
            remarks = st.text_input(
                "Allocation remarks",
                key=(
                    f"allocator_remarks_{request_id}"
                ),
            )
            ...
            if st.button(
                ...
                type="primary",
                key=(
                    f"allocate_vehicle_{request_id}"
                ),
            ):
                ...
                if not vehicle_is_available(
                    ...
                    st.error(
                        ...
                        "to another request. Please select "
                        "another vehicle."
                    )
                ...
                update_request(
                    request_id,
                    {
                        "status": "Pending HR",
                        "start_time": (
                            requested_start_dt.strftime(
                                "%H:%M"
                            )
                        ),
                        "end_time": (
                            calculated_end_dt.strftime(
                                "%H:%M"
                            )
                        ),
                        "vehicle_number": (
    ...
    if pending_release.empty:
        ...
        for _, row in pending_release.iterrows():
            ...
            with st.expander(
                ...
                st.info(
                    "Security does not assign the vehicle "
                    "or driver. Both are already defined "
                ...
                if st.button(
                    ...
                    key=(
                        f"security_release_{request_id}"
                    ),
                ):
            ...
            with st.expander(
                ...
                if st.button(
                    ...
                    if distance is None:
                        ...
                        st.error(
                            "Cannot close the trip because "
                            "End Mileage must be greater than "
                            "or equal to Start Mileage."
                        )

                    else:

                        # Correct distance is explicitly
                        # recalculated here before closing.
                        update_request(
                            request_id,
                            {
                        ...
                        audit(
                            ...
                            (
                                f"Start mileage: "
                                f"{format_mileage(row.get('start_mileage'))} km; "
                                f"End mileage: "
                                f"{format_mileage(row.get('end_mileage'))} km; "
                                f"Distance: "
                                f"{format_mileage(distance)} km; "
                                f"{remarks}"
                            ),
    ...
    for _, row in assigned.iterrows():
        ...
        with st.expander(
            ...
            if status == "Vehicle Released":
                ...
                if st.button(
                    ...
                    if start_mileage <= 0:
                        ...
                        audit(
                            ...
                            (
                                f"Start mileage: "
                                f"{format_mileage(start_mileage)} km"
                            ),
                        )
                ...
                st.info(
                    "Recorded starting mileage: "
                    f"{format_mileage(start_value)} km"
                )
                        ...
                        if distance is None:
                            ...
                            audit(
                                ...
                                (
                                    ...
                                    f"{format_mileage(start_value)} km; "
                                    f"End mileage: "
                                    f"{format_mileage(end_mileage)} km; "
                                    f"Distance: "
                                    f"{format_mileage(distance)} km"
                                ),
                            )
                            ...
                            st.success(
                                "Trip completed and sent "
                                "to Security for verification."
                            )
                ...
                try:
                    ...
                    # Always recalculate instead of trusting
                    # an old/wrong distance_km cell.
                    correct_distance = (
                    ...
                    st.write(
                        f"End mileage: "
                        f"{format_mileage(end_value)} km"
                    )

                    st.write(
                        f"Distance: "
                        f"{format_mileage(correct_distance)} km"
                    )
                    ...
                    st.write(
                        "Distance: Invalid mileage values"
                    )
...
def admin_portal():
    ...
    # ========================================================
    # CORRECT DISTANCE DISPLAY EVERYWHERE
    # ========================================================
    # Recalculate Distance from:
    #
    #     End Mileage - Start Mileage
    #
    # instead of relying on potentially incorrect
    # stored distance_km values.
    # ========================================================

    display_dataframe = dataframe.copy()

    if (
        "start_mileage" in display_dataframe.columns
        and "end_mileage" in display_dataframe.columns
    ):

        display_dataframe["distance_km"] = (
    ...
    if travel_dates:
        ...
        selected_records = display_dataframe[
            display_dataframe[
                "travel_date"
            ].apply(parse_date)
        ...
        if selected_records.empty:
            ...
            st.download_button(
                label=(
                    "📥 Download Gate Pass History as Excel"
                ),
