...
def create_required_sheets():
    ...
    departments = get_worksheet(
        "Departments"
    )

    if len(
        departments.get_all_values()
    ) <= 1:
    ...
    drivers = get_worksheet(
        "Drivers"
    )

    if len(
        drivers.get_all_values()
    ) <= 1:
    ...
    vehicles = get_worksheet(
        "Vehicles"
    )

    if len(
        vehicles.get_all_values()
    ) <= 1:
